...
echo "$($PSQL "SELECT name FROM teams FULL JOIN games ON teams.team_id = games.winner_id WHERE winner_id IS NOT NULL AND year=2018 AND round='Final';")"
...
echo "$($PSQL "SELECT year, name FROM teams INNER JOIN games ON teams.team_id = games.winner_id WHERE winner_id IS NOT NULL AND round='Final' ORDER BY year;")"
